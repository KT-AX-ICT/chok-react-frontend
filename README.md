...
React 프론트엔드는 Spring Boot API에서 제공하는 BGL 로그, 라벨 기반 정상/이상 분류 결과, AI 분석 결과, 반복 패턴, 대시보드 지표를 화면에 표시합니다. BGL 로그 seed data 적재, 분석 결과 저장 및 조회, Scheduler 실행, FastAPI 호출 연동은 Spring Boot 영역입니다. AI 근거 생성과 반복 패턴 분석 로직 자체는 FastAPI/Python 영역입니다.
...
| `VITE_USE_MOCKS` | `true` | 백엔드 API가 준비되기 전 로컬 mock data 사용 여부 |
...
- BGL 첫 번째 라벨 컬럼 기준 정상/이상 로그 목록 조회
- 라벨상 이상 로그의 AI 분석 결과 목록 조회
- 분석 상세에서 원본 로그, 위험도, 근거 설명, 대응 방안 확인
- 누적 이상 로그의 반복 패턴 클러스터 조회
- Spring Boot API 연동 전까지 mock data 기반 화면 검증

정상/이상 분류는 프론트엔드가 새로 판단하지 않습니다. BGL 로그 첫 번째 컬럼의 라벨을 기준으로 분류하고, 프론트엔드는 라벨상 이상 로그에 대한 AI 근거 설명을 표시합니다.
...
- React는 정상/이상 여부를 새로 판단하지 않고 Spring Boot에서 제공하는 라벨 기반 분류 결과를 사용합니다.
...
- 실제 Spring Boot API 계약 확정 후 응답 DTO 조정
- 서버 페이지네이션/정렬/필터 연동
- 실시간 갱신 또는 Scheduler 상태 표시
- 인증/인가 UI
- 접근성/반응형 QA
- E2E 테스트
