...
- Playwright 가 dev 서버를 **`VITE_USE_MOCKS=false`** 로 **전용 포트 `5180`** 에 자동
  기동합니다(`playwright.config.ts` 의 `webServer`). Vite 는 `process.env` 의 `VITE_*`
  변수를 `.env` 파일보다 우선 적용하므로 `.env` 를 건드리지 않아도 됩니다.
  - 개발용으로 `5173` 에 직접 서버를 띄워 둔 채로 **동시에** E2E 를 돌려도 포트가
    달라 충돌하지 않습니다(개발 서버는 그대로 두면 됩니다).
  - 포트 변경: `E2E_PORT=5181 npm run test:e2e`
...
> **ℹ️ 포트 분리 (개발 서버와 동시 실행)**
> E2E/데모는 개발용(보통 5173)과 **다른 전용 포트 `5180`** 에 Playwright 가 직접
> `VITE_USE_MOCKS=false` 로 서버를 띄웁니다. 덕분에 개발 서버를 끌 필요 없이 동시에
> 돌릴 수 있고, 예전처럼 5173 의 mock 서버를 잘못 재사용해 필터 검증이 깨지는 일도 없습니다.
> 단, **5180 을 다른 용도로 이미 쓰고 있다면** `E2E_PORT` 로 비켜 주세요
> (`E2E_PORT=5181 npm run test:e2e`). `--strictPort` 라 포트가 점유돼 있으면 조용히
> 다른 포트로 가지 않고 명확히 실패합니다.
